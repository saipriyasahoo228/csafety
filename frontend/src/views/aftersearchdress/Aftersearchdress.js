...
export default function Aftersearchdress(props) {
  ...
  const handlePdfDownload = () => {
    ...
    // Table Headers
    const headers = [['SL NO.', 'REF NO.', 'DRESS LIST', 'ISSUED DATE', 'NEXT ISSUANCE', 'IMAGE', 'DESCRIPTION']];
    ...
    // Generate AutoTable with Increased Row Height & Image Handling
    doc.autoTable({
        head: headers,
        body: body,
        startY: 20,
        ...
        styles: {
            fontSize: 10,
            cellPadding: 8, // Increased padding for better row height
            valign: "middle",
            halign: "center",
        },
        headStyles: {
        ...
        columnStyles: {
            0: { cellWidth: 23 }, // SL NO.
            1: { cellWidth: 28 }, // REF NO.
            2: { cellWidth: 45 }, // DRESS LIST
            3: { cellWidth: 38 }, // ISSUED DATE
            4: { cellWidth: 38 }, // NEXT ISSUANCE
            5: { cellWidth: 48 }, // IMAGE
            6: { cellWidth: 65 }, // DESCRIPTION
        },
        margin: { right: 20 }, // Adds space on the right side
        didParseCell: (data) => {
            if (data.section === "body") {
                data.cell.styles.minCellHeight = 25; // Increase row height
            }
        },
        didDrawCell: (data) => {
            if (data.column.index === 5) { // Image column
