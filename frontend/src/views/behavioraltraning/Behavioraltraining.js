...
export default function Behavioraltraining(props) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await api.get(`/reports/employee/training/?filter=behavioral&employeeId=${employee_id}`,

        );
        // console.log(response.data);
        const data = response.data.data['BEHAVIORAL TRAINING'];
        console.log('result:', data);

        const tableRowsData = data.map((training, index) => ({
          slno: index + 1,
          // behavioraltraining: training.training_name || "NA",
  ...
  //   });
  //   doc.save('Behavioral_Training_Report.pdf');
  // };

  const handlePdfDownload = () => {
    const doc = new jsPDF({ orientation: "landscape" }); // Landscape mode for better layout

    // Table Headers (Without Image)
    const headers = [['SL NO.', 'DATE', 'TRAINER NAME', 'REFERENCE NUMBER', 'DESCRIPTION', 'IMAGE']];

    // Table Body with Placeholder for Image
    ...
    doc.autoTable({
        ...
        columnStyles: {
            ...
            3: { cellWidth: 45 },
            4: { cellWidth: 60 },
            5: { cellWidth: 50 }, // Image Column
        },
        margin: { right: 15 },
        didParseCell: (data) => {
            if (data.section === "body") {
                data.cell.styles.minCellHeight = 25; // Increase row height for better spacing
            }
        },
        didDrawCell: (data) => {
            // Only process images in the body, not in the headers
