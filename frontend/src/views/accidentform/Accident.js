...
const Accident = () => {
  ...
  useEffect(() => {
    if (toolbox_reference_number && toolbox_reference_number.trim() !== "") {
      setToolboxTrain('true'); // Set to 'Yes' if there's a reference number
    } else {
      setToolboxTrain('false'); // Otherwise, set to 'No'
    }
  }, [toolbox_reference_number]); // Runs whenever toolbox_reference_number updates
...
const handleSubmit = async (event) => {
  ...
  const handleUpdate = async (event) => {
    ...
    try {
      ...
      alert('Updated Accident successfully:', response.data);
      resetForm();
...
<Grid item xs={12}>
  ...
  <FormControl component="fieldset">
    ...
    <RadioGroup 
      row 
      value={toolbox_train} 
      onChange={(e) => setToolboxTrain(e.target.value)}
    >
      <FormControlLabel value="true" control={<Radio />} label="Yes" />
      <FormControlLabel value="false" control={<Radio />} label="No" />
    </RadioGroup>
