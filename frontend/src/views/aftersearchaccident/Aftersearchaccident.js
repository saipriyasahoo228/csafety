...
export default function Aftersearchaccident(props) {
  ...
  const handlePdfDownload = () => {
    ...
    const lineSpacing = 7; // Space between each text line
    const sectionSpacing = 10; // Space between records
    const marginLeft = 10;
    ...
    rows.forEach((row, index) => {
        ...
        // Write each field with proper spacing
        doc.text(`SL NO.: ${row.slno || "N/A"}`, marginLeft, y);
        doc.text(`REFERENCE NUMBER: ${row.reference_number || "N/A"}`, marginLeft, y + lineSpacing);
        doc.text(`DATE: ${row.date || "N/A"}`, marginLeft, y + lineSpacing * 2);
        doc.text(`Permit Status: ${row.permit_status || "N/A"}`, marginLeft, y + lineSpacing * 3);
        doc.text(`PPE Status: ${row.ppe_status || "N/A"}`, marginLeft, y + lineSpacing * 4);
        doc.text(`Severity ID: ${row.severity_id || "N/A"}`, marginLeft, y + lineSpacing * 5);
        doc.text(`Severity: ${row.severity || "N/A"}`, marginLeft, y + lineSpacing * 6);
        doc.text(`About: ${row.about || "N/A"}`, marginLeft, y + lineSpacing * 7);
        doc.text(`ToolBox Talk: ${row.toolbox_training || "N/A"}`, marginLeft, y + lineSpacing * 8);
        doc.text(`ToolBox Reference: ${row.toolbox_training_reference_number || "N/A"}`, marginLeft, y + lineSpacing * 9);

        // Image Handling
        if (row.accident_image) {
            const imgData = `data:image/png;base64,${row.accident_image}`;
            doc.text("Image:", marginLeft, y + lineSpacing * 10);
            doc.addImage(imgData, "PNG", marginLeft + 25, y + lineSpacing * 10, 30, 30);
            y += 40; // Increase space for image
        } else {
            doc.text("No Image Available", marginLeft, y + lineSpacing * 10);
        }

        // Add spacing between records
        y += sectionSpacing + lineSpacing * 10;
    });
