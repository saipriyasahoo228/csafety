...
export default function Jobsafetytraining(props) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await api.get(`reports/employee/training/?filter=safety&employeeId=${employee_id}`,

        );
        // console.log(response.data);
        const data = response.data.data['JOB SAFETY TRAINING'];
