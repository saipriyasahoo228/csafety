...
export default function Aftersearchviolation() {
  ...
  const handlePdfDownload = () => {
    ...
    rows.forEach((row, index) => {
        ...
        if (yPos > 180) {
            doc.addPage();
            yPos = 20; // Reset position
        }

        // Print each field in a structured format
        doc.text(`SL NO.: ${row.slno || "N/A"}`, 10, yPos);
        doc.text(`REFERENCE NUMBER: ${row.reference_number || "N/A"}`, 10, yPos + 6);
        doc.text(`DATE: ${row.date || "N/A"}`, 10, yPos + 12);
        doc.text(`ABOUT: ${row.about || "N/A"}`, 10, yPos + 18);

        // Check for image data
        if (row.accident_image) {
            const imgData = `data:image/png;base64,${row.accident_image}`;
            doc.addImage(imgData, 'PNG', 150, yPos, 40, 25); // Adjust image position & size
        } else {
            doc.text(`IMAGE: N/A`, 150, yPos + 12);
        }
