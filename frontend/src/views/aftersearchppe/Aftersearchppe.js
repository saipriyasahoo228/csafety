...
export default function Aftersearchdress(props) {
  ...
  const handlePdfDownload = () => {
    ...
    const headers = [
      ['SERIAL NO.', 'REF NO.', 'PPE LIST', 'ISSUED DATE', 'NEXT ISSUANCE', 'DESCRIPTION', 'IMAGE']
    ];
    ...
    const body = rows.map(row => [
      ...
      '' // Empty string to prevent IMAGE header issue
    ]);

    // Generate AutoTable with Image Handling
    doc.autoTable({
      head: headers,
      body: body,
      startY: 20,
      ...
      columnStyles: {
        ...
        2: { cellWidth: 50 }, // PPE LIST
        3: { cellWidth: 40 }, // ISSUED DATE
        4: { cellWidth: 40 }, // NEXT ISSUANCE
        5: { cellWidth: 70 }, // DESCRIPTION
        6: { cellWidth: 40 }, // IMAGE COLUMN (reduced to fit)
      },
      didParseCell: (data) => {
        if (data.section === "body") {
          data.cell.styles.minCellHeight = 45; // Increased row height for better visibility
        }
      },
      didDrawCell: (data) => {
        // ✅ Check if it's a body cell, not the header
        if (data.section === "body" && data.column.index === 6 && rows[data.row.index].newIssuance_image) {
          const imgData = `data:image/png;base64,${rows[data.row.index].newIssuance_image}`;

          // Set Image Size (Adjusted for better fit)
          const imgWidth = 35;
          const imgHeight = 35;
          const xPos = data.cell.x + (data.cell.width - imgWidth) / 2; // Center the image
