...
export default function AccidentReportDashboard() {
    ...
    const fetchData = async () => {
      try {
        ...
        const formattedData = data.results.map((item, index) => ({
          ...
          training_id:item.training_id|| "N/A",
  ...
  const handleDownload = async () => {
    ...
    worksheet.columns = [
      ...
      { header: 'ToolBox Reference_number', key: 'ToolBox_Reference_number' },
    ...
    filteredRows.forEach((row) => {
      worksheet.addRow({
        ...
        toolbox_reference_number_id:row.training_id,
  ...
  const downloadRowPDF = async () => {
    ...
    content.style.backgroundColor = 'white'; // Ensure white background
    ...
    content.innerHTML = `
      <div style="font-family: Arial, sans-serif; font-size: 12px; padding: 20px;">
        ...
        <table border="1" cellpadding="5" cellspacing="0" style="width: 100%; margin: 10px 0;">
          ...
          <tr><td>Accident Report Date</td><td>${selectedRow.accident_report_date || 'N/A'}</td></tr>
          <tr><td>Accident Type</td><td>${selectedRow.accident_type === 1 ? 'Near Miss' :
            selectedRow.accident_type === 2 ? 'Accident' :
            selectedRow.accident_type === 3 ? 'Violation' : 'Unknown'}</td></tr>
          <tr><td>Severity</td><td>${selectedRow.severity || 'N/A'}</td></tr>
          <tr><td>Accident ID</td><td>${selectedRow.accident_id || 'N/A'}</td></tr>
          <tr><td>Workmen Count</td><td>${selectedRow.workmen_count || 0}</td></tr>
          <tr><td>Workmen Names</td><td>${selectedRow.workmen_names || 'N/A'}</td></tr>
          <tr><td>ReportedBy Count</td><td>${selectedRow.reportedby_count || 0}</td></tr>
          <tr><td>ReportedBy Names</td><td>${selectedRow.reportedby_names || 'N/A'}</td></tr>
          <tr><td>Supervisor Count</td><td>${selectedRow.supervisor_count || 0}</td></tr>
          <tr><td>Supervisor Names</td><td>${selectedRow.supervisor_names || 'N/A'}</td></tr>
          <tr><td>ToolBox Reference Number Id:</td><td>${selectedRow.training_id || 'N/A'}</td></tr>
          <tr><td>Permit Status</td><td>${selectedRow.permit_status_id === 1 ? 'Valid' :
            selectedRow.permit_status_id === 2 ? 'Not Required' :
            selectedRow.permit_status_id === 3 ? 'No Permit' :
            selectedRow.permit_status_id === 4 ? 'Expired' : 'Unknown'}</td></tr>
          <tr><td>PPE Status</td><td>${selectedRow.ppe_status_id === 1 ? 'OK' :
            selectedRow.ppe_status_id === 2 ? 'Faulty' : 'Unknown'}</td></tr>
          <tr><td>Description</td><td>${selectedRow.about_the_accident || 'N/A'}</td></tr>
        </table>
        <div style="text-align: center; margin-bottom: 20px;">
          <img src="data:image/png;base64,${selectedRow.accident_image}" alt="Accident Image" 
    ...
    setTimeout(async () => {
      ...
      doc.save(`Accident_Report_${selectedRow.accident_id}.pdf`);
    }, 500); // Add small delay
  };
 ...
 return (
              ...
              {visibleRows.map((row, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell align="left">{row.training_id}</TableCell>
                    ...
                    <TableCell align="left"  sx={{
                                    ...
                                    maxHeight: '50px',  // Limits the height to make sure scrollbar appears
                                    whiteSpace: 'normal',  // Allows wrapping text
                                    wordWrap: 'break-word' // Breaks long words if necessary
                    }}>{row.about_the_accident}</TableCell>
                    <TableCell align="right">
                    <img
  ...
  <DialogContent>
    {selectedRow && (
      <Box >
        ...
        <Typography variant="body1"><strong>Accident TYPE:</strong>  {selectedRow.accident_type === 1 ? 'Near Miss' : selectedRow.accident_type === 2 ? 'Accident' : selectedRow.accident_type === 3 ? 'Violation' : 'Unknown'}</Typography>
        ...
        <Typography variant="body1"><strong>PPE Status:</strong>  {selectedRow.ppe_status_id ===1?'OK':
                      selectedRow.ppe_status_id==2?'Faulty':
                      'Unknown' }</Typography>
        <Typography variant="body1"><strong>Permit Status:</strong> {selectedRow.permit_status_id === 1 ? 'Valid' :
            selectedRow.permit_status_id === 2 ? 'Not Required' :
            selectedRow.permit_status_id === 3 ? 'No Permit' :
            selectedRow.permit_status_id === 4 ? 'Expired' : 'Unknown'}</Typography>
        {/* <Typography variant="body1"><strong>ToolBOx Reference Number:</strong> {selectedRow.toolbox_reference_number_id}</Typography> */}
        <Typography variant="body1"><strong>ToolBOx Reference Number:</strong> {selectedRow.training_id}</Typography>
  ...
  <DialogActions>
    ...
    <button onClick={downloadRowPDF} style={{ padding: '10px' }}>Download</button>
  </DialogActions>
</Dialog>
