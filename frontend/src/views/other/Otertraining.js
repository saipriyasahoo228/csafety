...
const handlePdfDownload = () => {
  ...
  rows.forEach((row, index) => {
      if (y > 260) { // Prevents content overflow, moves to next page
          doc.addPage();
          y = 20;
      }

      doc.setFontSize(9); // Decrease font size to avoid overlap
      doc.setTextColor(0, 0, 0);

      doc.setFont(undefined, "bold");
      doc.text(`SL NO.:`, 10, y);
      doc.setFont(undefined, "normal");
      doc.text(row?.slno ? row.slno.toString() : "N/A", 50, y);

      y += 7;
      doc.setFont(undefined, "bold");
      doc.text(`OTHER TRAINING:`, 10, y);
      doc.setFont(undefined, "normal");
      doc.text(row?.othertraining || "N/A", 50, y);

      y += 7;
      doc.setFont(undefined, "bold");
      doc.text(`DATE:`, 10, y);
      doc.setFont(undefined, "normal");
      doc.text(row?.date || "N/A", 50, y);

      y += 7;
      doc.setFont(undefined, "bold");
      doc.text(`TRAINER NAME:`, 10, y);
      doc.setFont(undefined, "normal");
      doc.text(row?.trainername || "N/A", 50, y);

      y += 7;
      doc.setFont(undefined, "bold");
      doc.text(`REFERENCE NUMBER:`, 10, y);
      doc.setFont(undefined, "normal");
      doc.text(row?.refrno || "N/A", 50, y);

      y += 7;
      doc.setFont(undefined, "bold");
      doc.text(`DESCRIPTION:`, 10, y);
      doc.setFont(undefined, "normal");
      doc.text(row?.description || "N/A", 50, y, { maxWidth: 140 });

      y += 10; // Extra space before the image

      if (row?.training_image) {
          try {
              const imgData = `data:image/png;base64,${row.training_image}`;

              const imgWidth = 40;
              const imgHeight = 40;

              if (y + imgHeight > 270) { // Prevents image from getting cut-off
                  doc.addPage();
                  y = 20;
              }
      ...
      y += 7; // Space between records
      doc.line(10, y, 200, y); // Adds a separator line
      y += 7;
  });
