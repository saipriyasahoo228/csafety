...
const headCells = [
  ...
  { id: 'accident_image', numeric: true, disablePadding: false, label: 'IMAGE' },
...
export default function Nearmissreport(props) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await api.get(`/reports/employee/accident/?filter=nearmiss&employeeId=${employee_id}`);
  ...
  //   const doc = new jsPDF();
  //   doc.autoTable({
  //     head: [['SL NO.', 'REFERENCE NUMBER', 'DATE','ABOUT']],
  //     body: rows.map((row) => [row.slno, row.reference_number, row.date,row.about]),
  //   });
  //   doc.save('Nearmiss_Report.pdf');
  // };

  const handlePdfDownload = () => {
    const doc = new jsPDF({ orientation: "landscape" }); // Landscape for better spacing

    // Table Headers
    const headers = [['SL NO.', 'REFERENCE NUMBER', 'DATE', 'IMAGE', 'ABOUT']];

    // Table Body - Ensure missing data is replaced with "N/A"
    const body = (Array.isArray(rows) && rows.length > 0) ? rows.map(row => [
        row?.slno || "N/A",
        row?.reference_number || "N/A",
        row?.date || "N/A",
        "", // Placeholder for Image
        row?.about || "N/A"
    ]) : [["N/A", "N/A", "N/A", "N/A", "N/A"]]; // If no data, create a single row with "N/A"

    // Generate AutoTable with proper styling
    doc.autoTable({
        head: headers,
        body: body,
        startY: 20,
        ...
        styles: {
            fontSize: 10,
            cellPadding: 8, // More spacing for readability
            valign: "middle",
            halign: "center",
        },
        headStyles: {
        ...
        columnStyles: {
            ...
            1: { cellWidth: 45 },  // REFERENCE NUMBER
            2: { cellWidth: 30 },  // DATE
            3: { cellWidth: 50 },  // IMAGE
            4: { cellWidth: 60 },  // ABOUT
        },
        didDrawCell: (data) => {
            if (data.column.index === 3 && Array.isArray(rows) && rows.length > 0) { // Image Column
                const rowIndex = data.row.index;
                if (rows[rowIndex]?.accident_image) {
                    const imgData = `data:image/png;base64,${rows[rowIndex].accident_image}`;

                    // Fit image within the cell
                    const cellWidth = data.cell.width;
                    const cellHeight = data.cell.height;
                    doc.addImage(imgData, 'PNG', data.cell.x + 2, data.cell.y + 2, cellWidth - 4, cellHeight - 4);
                } else {
                    // Show "N/A" if no image is available
                    doc.text("N/A", data.cell.x + data.cell.width / 2 - 5, data.cell.y + data.cell.height / 2);
                }
            }
        }
    });
